...
*/
// 
void DSRstack(int nt, double *attr, double *mx, double *my, double *hx, double *hy, double *mxmx, double *mxmy, double *mymy, double *hxhx, double *hxhy, double *hyhy, double *tti)
{
    int tnum; 
    double t0; 
    ...
    {
    /////////////////////////////////////////////////////    
        double mNm = attr[5]*mxmx[tnum]+2*attr[6]*mxmy[tnum]+attr[7]*mymy[tnum];
        double hMh = attr[2]*hxhx[tnum]+2*attr[3]*hxhy[tnum]+attr[4]*hyhy[tnum];
        double mNh = 2*attr[5]*mx[tnum]*hx[tnum]+2*attr[6]*(mx[tnum]*hy[tnum]+my[tnum]*hx[tnum])+2*attr[7]*my[tnum]*hy[tnum];

        double ts0 = t0+attr[0]*(mx[tnum]-hx[tnum])+attr[1]*(my[tnum]-hy[tnum]);
        double ts1 = ts0*ts0 + 2*t0*(mNm - mNh + hMh);

        double tg0 = t0+attr[0]*(mx[tnum]+hx[tnum])+attr[1]*(my[tnum]+hy[tnum]);
        double tg1 = tg0*tg0 + 2*t0*(mNm + mNh + hMh);
  
        //  return min(ts1,tg1)>0?0.5*(sqrt(ts1)+sqrt(tg1)):-1.0;
        tti[tnum] = (sqrt(ts1)+sqrt(tg1))/2;
    //////////////////////////////////////////////////////       
    }
    return;
}
...
DSRstack(nt,attr,mx,my,hx,hy,mxmx,mxmy,mymy,hxhx,hxhy,hyhy,tti);
return;
}