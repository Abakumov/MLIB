function [ p, ux, uz ] = Get_FD_forward_test( G, acq, shotnum, fpeak, vp, rho, vs)
...
%   25.06.2015
    ...
    xs   = round(acq.sx(shotnum));              % x coordinates of shots (in meters)
    zs   = round(acq.sz(shotnum));              % z coordinates of shots (in meters)
    xr   = round(acq.rx);                       % x coordinates of receivers (in meters)
    zr   = round(acq.rz);                       % z coordinates of receivers (in meters)
