// FSM-type solver of adjoint eikonal equation, 2D
// version: 1.0
// Abakumov Ivan
...
double localasm(int i, int j, double **tti, double **lambda, double **DCR, double *G)
{
  ...
  ap = -(tti[i+1][j]-tti[i][j])/Gdx; app=(ap+fabs(ap))/2.; amp=(ap-fabs(ap))/2.;
  am = -(tti[i][j]-tti[i-1][j])/Gdx; apm=(am+fabs(am))/2.; amm=(am-fabs(am))/2.;
  bp = -(tti[i][j+1]-tti[i][j])/Gdz; bpp=(bp+fabs(bp))/2.; bmp=(bp-fabs(bp))/2.;
  bm = -(tti[i][j]-tti[i][j-1])/Gdz; bpm=(bm+fabs(bm))/2.; bmm=(bm-fabs(bm))/2.;
  
  coef = (app-amm)/Gdx + (bpp-bmm)/Gdz;
  if (coef!=0){
  nlambda = (apm*lambda[i-1][j] - amp*lambda[i+1][j])/Gdx/coef + (bpm*lambda[i][j-1] - bmp*lambda[i][j+1])/Gdz/coef + DCR[i][j]/coef; 
  }
...
void Getlambda(double *G, double **tti, double **DCR, double **lambda, int *MXYZ)
{
  ...
  double Gdx,Gdz;         // Initial point and steps
  ...
  for (i=MINX;i<MAXX;i++){
    for (j=MINZ;j<MAXZ;j++){
      lambda[i][j]=DCR[i][j]/(-(tti[i][j+1]-tti[i][j])/Gdz);   //normal_up = {0, 0, -1}    (px*n1+py*n2+pz*n3)  up
    }
  }
  ...
  for (q=0;q<2;q++){
    ...
    for (i=MINX;i<MAXX;i++){
      for (j=MINZ;j<MAXZ;j++){
        lambda[i][j]=localasm(i,j,tti,lambda,DCR,G);
      }                           
    }
      //#2
    for (i=MINX;i<MAXX;i++){
      for (j=MAXZ-1;j>MINZ-1;j--){
        lambda[i][j]=localasm(i,j,tti,lambda,DCR,G);
      }                           
    }
      //#3
...
void ASM(double *G, double *traveltime, double *rightside, double *out)
{
  ...
  for (i=0;i<Gnx;i++){
    tti[i] = new double[Gnz];
    dcr[i] = new double[Gnz];
    lambda[i] = new double[Gnz];
  }
  for (i=0;i<Gnx;i++){
    for (j=0;j<Gnz;j++){
      tti[i][j] = traveltime[i+j*Gnx];  
      dcr[i][j] = rightside[i+j*Gnx];  
      lambda[i][j] = 0;
    }
  }
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
    ...
    if (rndim!=2) mexErrMsgTxt("Number of dimensions of DCR array is not correct.");
    if (info!=0) mexPrintf("rdims: rdims[0]=%d, rdims[1]=%d\n",rdims[0],rdims[1]);    
    if (rdims[0]!=(int)G[3] || rdims[1]!=(int)G[5]) mexErrMsgTxt("Size of dcr array is not correctly defined in G-array");
    
    
    ndim = mxGetNumberOfDimensions(prhs[2]);
    dims = mxGetDimensions(prhs[2]);   
    if (ndim!=2) mexErrMsgTxt("Number of dimensions of tti array is not correct.");
    if (info!=0) mexPrintf("dims: dims[0]=%d, dims[1]=%d\n",dims[0],dims[1],dims[2]);    
    if (dims[0]!=(int)G[3] || dims[1]!=(int)G[5]) mexErrMsgTxt("Size of tti array is not correctly defined in G-array");
